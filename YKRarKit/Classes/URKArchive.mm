...
- (NSString *)errorNameForErrorCode:(NSInteger)errorCode detail:(NSString * __autoreleasing *)errorDetail
{
    ...
    switch (errorCode) {
        ...
        default:
            errorName = [NSString stringWithFormat:@"未知错误 (%ld)", (long)errorCode];
            detail = [NSString stringWithFormat:@"遇到未知错误（错误代码：%ld）", (long)errorCode];
            break;
    }
