...
mkdir    Layout/var/jb/Applications
# 创建 Library 目录，用于存放库文件
mkdir    Layout/var/jb/Library
# 创建 MobileSubstrate 目录，用于存放移动基板相关文件
mkdir    Layout/var/jb/Library/MobileSubstrate
# 创建 DynamicLibraries 目录，用于存放动态库文件
mkdir    Layout/var/jb/Library/MobileSubstrate/DynamicLibraries
# 创建 LaunchDaemons 目录，用于存放启动守护程序文件
mkdir    Layout/var/jb/Library/LaunchDaemons
...
echo "拷贝文件..."
cp -rf  Products/YKApp.app                                 Layout/var/jb/Applications/YKApp.app
mkdir -p Layout/var/jb/Applications/YKApp.app/bin

cp      Products/YKSBTweak.dylib                           Layout/var/jb/Library/MobileSubstrate/DynamicLibraries/YKSBTweak.dylib
cp      ../YKSBTweak/YKSBTweak.plist                       Layout/var/jb/Library/MobileSubstrate/DynamicLibraries/YKSBTweak.plist

cp      Products/YKUITweak.dylib                           Layout/var/jb/Library/MobileSubstrate/DynamicLibraries/YKUITweak.dylib
cp      Plist/YKUITweak.plist                              Layout/var/jb/Library/MobileSubstrate/DynamicLibraries/YKUITweak.plist

cp      Products/YKMediaserverdTweak.dylib                 Layout/var/jb/Library/MobileSubstrate/DynamicLibraries/YKMediaserverdTweak.dylib
cp      Plist/YKMediaserverdTweak.plist                    Layout/var/jb/Library/MobileSubstrate/DynamicLibraries/YKMediaserverdTweak.plist

cp      Products/YKLaunchd                                 Layout/var/jb/Applications/YKApp.app/bin/YKLaunchd
cp      ../YKLaunchd/YKLaunchdRootless.plist               Layout/var/jb/Library/LaunchDaemons/com.sky.yklaunchd.plist

cp      Products/YKService                                 Layout/var/jb/Applications/YKApp.app/bin/YKService
echo "拷贝文件完成..."

echo "ldid 签名..."
ldid -S                                                    Layout/var/jb/Library/MobileSubstrate/DynamicLibraries/YKSBTweak.dylib
ldid -S                                                    Layout/var/jb/Library/MobileSubstrate/DynamicLibraries/YKUITweak.dylib
ldid -S                                                    Layout/var/jb/Library/MobileSubstrate/DynamicLibraries/YKMediaserverdTweak.dylib
ldid -SSignXml/YKApp.entitlements                          Layout/var/jb/Applications/YKApp.app
ldid -SSignXml/YKLaunchd.entitlements                      Layout/var/jb/Applications/YKApp.app/bin/YKLaunchd
ldid -SSignXml/YKService.entitlements                      Layout/var/jb/Applications/YKApp.app/bin/YKService
echo "ldid 签名完成..."
...
Version: ${VER}
Icon: file:///var/jb/Applications/YKApp.app/AppIcon60x60@2x.png
...
# preinst 在软件包被安装之前执行。用于在安装软件包之前执行一些准备工作。例如，创建所需的目录结构，检查系统依赖项，设置权限等(这边要先杀掉剪切板进程,因为重启Springboard并不会重启剪切板，所以需要手动杀死)
...
rm -rf /var/jb/Applications/YKApp.app
rm -rf /var/jb/Library/LaunchDaemons/com.sky.yklaunchd.plist
...
if launchctl list | grep -q com.sky.yklaunchd; then
    ...
    launchctl unload /var/jb/Library/LaunchDaemons/com.sky.yklaunchd.plist
fi
